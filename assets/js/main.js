(() => {
  ...
  /* ============ CARD TILT ============ */
  if (!reduceMotion && window.matchMedia('(hover:hover) and (pointer:fine)').matches) {
    document.querySelectorAll('.skill-card, .project-card, .showcase-card, .stat-card').forEach(card => {
  ...
  document.querySelectorAll('.skills-grid, .projects-grid, .expertise-list, .stats-row, .showcase-grid').forEach(group => {
    Array.from(group.children).forEach((child, i) => child.style.setProperty('--i', i));
  });

  /* ============ STAT COUNTERS (Project Records) ============ */
  ...
  const statObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        ...
        const inc = Math.max(target / steps, 1);
        const timer = setInterval(() => {
          cur += inc;
          if (cur >= target) { cur = target; clearInterval(timer); }
          el.textContent = Math.round(cur);
        }, stepTime);
        statTimers.set(el, timer);
      } else {
        el.textContent = '0';
      }
    });
  ...
  /* ============ FLOATING QUICK NAV — scrollspy + sliding indicator ============ */
  const quickNav = document.getElementById('quickNav');
  const qnIndicator = document.getElementById('qnIndicator');
  const qnItems = Array.from(document.querySelectorAll('.qn-item'));
  const qnSections = ['home', 'skills', 'projects', 'about']
    .map(id => document.getElementById(id))
    .filter(Boolean);

  function moveIndicator(item) {
    if (!item) return;
    qnIndicator.style.width = item.offsetWidth + 'px';
    qnIndicator.style.transform = `translateX(${item.offsetLeft - 6}px)`;
  }

  function setActive(id) {
    qnItems.forEach(item => item.classList.toggle('active', item.dataset.target === id));
    const activeItem = qnItems.find(item => item.dataset.target === id);
  ...
  const qnObserver = new IntersectionObserver((entries) => {
    ...
    quickNav.classList.toggle('show', window.scrollY > window.innerHeight * 0.5);
  }, { threshold: 0.35, rootMargin: '-15% 0px -55% 0px' });
  qnSections.forEach(sec => qnObserver.observe(sec));
  ...
  window.addEventListener('resize', () => {
    const active = qnItems.find(item => item.classList.contains('active'));
    moveIndicator(active);
  });
  window.addEventListener('load', () => {
    setTimeout(() => moveIndicator(qnItems[0]), 300);
  });
